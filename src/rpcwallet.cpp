...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CBsgcoinAddress address(innerID);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"bsgcoinaddress\"    (string) The new Bsgcoin address\n"
            "\nExamples:\n"
    ...
    return CBsgcoinAddress(keyID).ToString();
}


CBsgcoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CBsgcoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"bsgcoinaddress\"   (string) The account Bsgcoin address\n"
            "\nExamples:\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"bsgcoinaddress\"  (string, required) The Bsgcoin address to be associated with an account.\n"
            "2. \"account\"         (string, required) The account to assign the address to.\n"
            "\nExamples:\n"
            + HelpExampleCli("setaccount", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\" \"tabby\"")
            + HelpExampleRpc("setaccount", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\", \"tabby\"")
        );

    CBsgcoinAddress address(params[0].get_str());
    if (!address.IsValid())
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"bsgcoinaddress\"  (string, required) The Bsgcoin address for account lookup.\n"
            "\nResult:\n"
            "\"accountname\"        (string) the account address\n"
            "\nExamples:\n"
            + HelpExampleCli("getaccount", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\"")
            + HelpExampleRpc("getaccount", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\"")
        );
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"bsgcoinaddress\"  (string) a Bsgcoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CBsgcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CBsgcoinAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"bsgcoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSent an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
           "1. \"bsgcoinaddress\"  (string, required) The Bsgcoin address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount inBSGto send. eg 0.1\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("sendtoaddress", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\" 0.1")
            + HelpExampleCli("sendtoaddress", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\" 0.1 \"donation\" \"seans outpost\"")
            + HelpExampleRpc("sendtoaddress", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\", 0.1, \"donation\", \"seans outpost\"")
        );
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"bsgcoinaddress\",     (string) The Bsgcoin address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CBsgcoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CBsgcoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CBsgcoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"bsgcoinaddress\"  (string, required) The Bsgcoin address to use for the private key.\n"
            ...
            + HelpExampleCli("signmessage", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\" \"my message\"") +
            "\nVerify the signature\n"
            + HelpExampleCli("verifymessage", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\" \"signature\" \"my message\"") +
            "\nAs json rpc\n"
            + HelpExampleRpc("signmessage", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\", \"my message\"")
        );
    ...
    CBsgcoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"bsgcoinaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given bsgcoinaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"bsgcoinaddress\"  (string, required) The Bsgcoin address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (numeric) The total amount inBSGreceived at this address.\n"
            "\nExamples:\n"
            "\nThe amount from transactions with at least 1 confirmation\n"
            + HelpExampleCli("getreceivedbyaddress", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\"") +
            "\nThe amount including unconfirmed transactions, zero confirmations\n"
            + HelpExampleCli("getreceivedbyaddress", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\" 0") +
            "\nThe amount with at least 10 confirmation, very safe\n"
            + HelpExampleCli("getreceivedbyaddress", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\" 10") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("getreceivedbyaddress", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\", 10")
       );

    // Bitcoin address
    CBsgcoinAddress address = CBsgcoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bsgcoin address");
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount inBSGreceived for this account.\n"
            "\nExamples:\n"
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01BSGfrom the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01BSGtimotei to akiko with a comment and funds have 10 confirmations\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tobsgcoinaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a Bsgcoin address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tobsgcoinaddress\"  (string, required) The Bsgcoin address to send funds to.\n"
            "3. amount                (numeric, required) The amount in BSG. (transaction fee is added on top).\n"
            ...
            "\"transactionid\"        (string) The transaction id.\n"
            "\nExamples:\n"
            "\nSend 0.01BSGfrom the default account to the address, must have at least 1 confirmation\n"
            + HelpExampleCli("sendfrom", "\"\" \"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\" 0.01") +
            "\nSend 0.01 from the tabby account to the given address, funds must have at least 10 confirmations\n"
            + HelpExampleCli("sendfrom", "\"tabby\" \"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\" 0.01 10 \"donation\" \"seans outpost\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("sendfrom", "\"tabby\", \"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\", 0.01, 10, \"donation\", \"seans outpost\"")
        );

    EnsureWalletIsUnlocked();

    string strAccount = AccountFromValue(params[0]);
    CBsgcoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bsgcoin address");
    CAmount nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The Bsgcoin address is the key, the numeric amount inBSGis the value\n"
            "      ,...\n"
            "    }\n"
            ...
            + HelpExampleCli("sendmany", "\"tabby\" \"{\\\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\\\":0.01,\\\"BfxZ9AoZ3cPT76ux9Zr4EXenQfRag2eWci\\\":0.02}\"") +
            "\nSend two amounts to two different addresses setting the confirmation and comment:\n"
            + HelpExampleCli("sendmany", "\"tabby\" \"{\\\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\\\":0.01,\\\"BfxZ9AoZ3cPT76ux9Zr4EXenQfRag2eWci\\\":0.02}\" 10 \"testing\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("sendmany", "\"tabby\", \"{\\\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\\\":0.01,\\\"BfxZ9AoZ3cPT76ux9Zr4EXenQfRag2eWci\\\":0.02}\", 10, \"testing\"")
        );
    ...
    set<CBsgcoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CBsgcoinAddress address(s.name_);
        if (!address.IsValid())
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of Bsgcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) Bsgcoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"bsgcoinaddress\"  (string) A Bsgcoin address associated with the keys.\n"

            "\nExamples:\n"
            "\nAdd a multisig address from 2 addresses\n"
            + HelpExampleCli("addmultisigaddress", "2 \"[\\\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\\\",\\\"BfxZ9AoZ3cPT76ux9Zr4EXenQfRag2eWci\\\"]\"") +
            "\nAs json rpc call\n"
            + HelpExampleRpc("addmultisigaddress", "2, \"[\\\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\\\",\\\"BfxZ9AoZ3cPT76ux9Zr4EXenQfRag2eWci\\\"]\"")
        ;
        throw runtime_error(msg);
    }
    ...
    return CBsgcoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CBsgcoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CBsgcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CBsgcoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount inBSGreceived by the address\n"
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"bsgcoinaddress\",    (string) The Bsgcoin address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in BSG. This is negative for the 'send' category, and for the\n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
       throw runtime_error(
            ...
            "    \"address\":\"bsgcoinaddress\",    (string) The Bsgcoin address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in BSG. This is negative for the 'send' category, and for the 'move' category for moves \n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"bsgcoinaddress\",   (string) The Bsgcoin address involved in the transaction\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending BSG\n"
            "\nArguments:\n"
...
Value walletlock(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 0))
        throw runtime_error(
            ...
            + HelpExampleCli("sendtoaddress", "\"Be9UAtaYHEk5cpsvNyg5r5GFtQGXyySf5b\" 1.0") +
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending Bsgcoin\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"bsgcoinaddress\" \"test message\"") +
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Bsgcoin stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid Bsgcoin stealth address."));
        return result;
    };
