...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBsgcoinAmount"))
        settings.setValue("nAnonymizeBsgcoinAmount", 1000);
    nAnonymizeBsgcoinAmount = settings.value("nAnonymizeBsgcoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBsgcoinAmount"))
        SoftSetArg("-anonymizebsgcoinamount", settings.value("nAnonymizeBsgcoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBsgcoinAmount:
            return QVariant(nAnonymizeBsgcoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBsgcoinAmount:
            nAnonymizeBsgcoinAmount = value.toInt();
            settings.setValue("nAnonymizeBsgcoinAmount", nAnonymizeBsgcoinAmount);
            emit AnonymizeBsgcoinAmountChanged(nAnonymizeBsgcoinAmount);
            break;
        default:
            break;
        }
