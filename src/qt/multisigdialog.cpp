...
void MultisigDialog::on_createAddressButton_clicked()
{
    ...
    for(int i = 0; i < ui->pubkeyEntries->count(); i++)
    {
        ...
        CBsgcoinAddress address(strAddressEntered);
    ...
    CBsgcoinAddress address(scriptID);
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                CBsgcoinAddress address(recipient.address.toStdString());
...
void MultisigDialog::on_sendTransactionButton_clicked()
{
    ...
    if(fee < minFee)
    {
        QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), tr("The fee of the transaction (%1 BSG) is smaller than the expected fee (%2 BSG). Do you want to send the transaction anyway?").arg((double) fee / COIN).arg((double) minFee / COIN), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
    else if(fee > minFee)
    {
        QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), tr("The fee of the transaction (%1 BSG) is bigger than the expected fee (%2 BSG). Do you want to send the transaction anyway?").arg((double) fee / COIN).arg((double) minFee / COIN), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
