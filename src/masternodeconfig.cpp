...
bool CMasternodeConfig::read(boost::filesystem::path path) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            if (!address.IsValid()) {
                LogPrintf("InvalidBSGaddress in masternode.conf line: %s\n", line.c_str());
                streamConfig.close();
                return false;
            }
        }
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(CService(ip).GetPort() != 36368) {
                LogPrintf("Invalid port detected in masternode.conf: %s (must be 36368 for mainnet)\n", line.c_str());
                streamConfig.close();
                return false;
            }
        } else if(CService(ip).GetPort() == 36368) {
            LogPrintf("Invalid port detected in masternode.conf: %s (36368 must be only on mainnet)\n", line.c_str());
            streamConfig.close();
            return false;
        }
