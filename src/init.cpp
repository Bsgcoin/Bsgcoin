...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: bsgcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: bsgcoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 36368)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 36369)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizebsgcoinamount=<n> " + _("Keep N Bsgcoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bsgcoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bsgcoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bsgcoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeBsgcoinAmount = GetArg("-anonymizebsgcoinamount", 0);
    if(nAnonymizeBsgcoinAmount > 999999) nAnonymizeBsgcoinAmount = 999999;
    if(nAnonymizeBsgcoinAmount < 2) nAnonymizeBsgcoinAmount = 2;
    ...
    LogPrintf("Anonymize Bsgcoin Amount %d\n", nAnonymizeBsgcoinAmount);
